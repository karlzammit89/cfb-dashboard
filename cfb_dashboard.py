...
if st.session_state.selected_cfbd_id:
    ...
    with btn_col4:
        if st.session_state.last_refresh:
            st.markdown(
                f"<div style='display:inline-flex;align-items:center;background:#2e7d32;"
                f"color:white;padding:0 16px;border-radius:8px;font-size:14px;"
                f"font-weight:700;white-space:nowrap;height:38px;box-sizing:border-box'>"
        ...
        for g in results:
            ...
            with st.container(border=True):
                ...
                if st.button(btn_label, key=f"pick_{g_id}", use_container_width=True,
                             ...
                             help="Data will be available once the game starts." if not has_started else None):
                    st.session_state.last_refresh = datetime.now(ET)
                    for k in ("cached_events", "cached_game_id", "filtered_events"):
                        st.session_state[k] = None
                    st.session_state.filters_applied    = False
